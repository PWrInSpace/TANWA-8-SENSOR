...
esp_err_t can_start(void) {
  ...
  // can_send_message(CAN_SENSOR_DATA_ID, NULL, 0); // Request sensor data on
  // startup
...
esp_err_t can_task_init(void) {
  ...
  if (xTaskCreatePinnedToCore(can_task, "CAN Task", CAN_TASK_STACK_SIZE, NULL,
                              9, &gb.task_handle, CAN_TASK_CORE_ID) != pdPASS) {
...
void handle_can_alerts(uint32_t alerts) {
  ...
  if (alerts & TWAI_ALERT_BUS_RECOVERED) {
    ...
    if (err == ESP_OK) {
      ...
      ESP_LOGE("CAN_DIAG", "Błąd restartu po recovery: %s",
               esp_err_to_name(err));
    }
    last_log_time = now;
  }
  ...
  if (alerts & TWAI_ALERT_ERR_PASS) {
    ESP_LOGW("CAN_DIAG",
             "Alert: ERROR PASSIVE! Sprawdź fizyczne połączenie. TEC: %lu",
             status.tx_error_counter);
    if (status.msgs_to_tx > 15)
      twai_clear_transmit_queue();
  }
  ...
  if (alerts & (TWAI_ALERT_TX_FAILED | TWAI_ALERT_BUS_ERROR)) {
    if (can_log_periodic) {
      // ESP_LOGW("CAN_DIAG", "Błędy magistrali (Brak ACK/BitError). TEC: %lu,
      // Oczekujące TX: %lu",
