...
esp_err_t parse_float_to_int16_t(float *input, size_t size, int16_t *output) {
  for (int i = 0; i < size; i++) {
    output[i] =
        input[i] * 100.f; // MAX 327BAR due to int16_t range <-32767 to 32767>
  }
  return ESP_OK;
}
...
esp_err_t can_config_init(void) {
  ...
  err = can_register_commands(can_commands,
                              sizeof(can_commands) / sizeof(can_commands[0]));
  if (err != ESP_OK) {
