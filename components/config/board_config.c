...
esp_err_t board_config_init(void) {
  ...
  max31856_init(&config.thermocouple[0], THERMOCOUPLE_CS1, MCU_SPI_TC_HOST);
  ESP_LOGI(TAG, "Thermocouple 1 DONE INIT");
  max31856_init(&config.thermocouple[1], THERMOCOUPLE_CS2, MCU_SPI_TC_HOST);
  ESP_LOGI(TAG, "Thermocouple 2 DONE INIT");
  max31856_init(&config.thermocouple[2], THERMOCOUPLE_CS3, MCU_SPI_TC_HOST);
...
#define EXPECTED_CALIBR_SIZE                                                   \
  (ADS1115_QUANTITY * PRESSURE_DRIVER_SENSOR_COUNT * VARIABLES_COUNT *         \
   sizeof(float))
  static_assert(sizeof(nvs_config.press_calibr) == EXPECTED_CALIBR_SIZE,
                "Struct padding detected! Pointer arithmetic in calibration "
                "loop for pressure sensors will fail.");

  for (int i = 0; i < ADS1115_QUANTITY; i++) {
    for (int j = 0; j < PRESSURE_DRIVER_SENSOR_COUNT; j++) {
      int idx = (i * (PRESSURE_DRIVER_SENSOR_COUNT * VARIABLES_COUNT)) +
                (j * VARIABLES_COUNT);
