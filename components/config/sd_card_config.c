...
#define SD_CSV_HEADER                                                          \
  ...
  "pressure0,pressure1,pressure2,pressure3,pressure4,pressure5,pressure6,"     \
  "pressure7,"                                                                 \
  "voltage0,voltage1,voltage2,voltage3,voltage4,voltage5,voltage6,voltage7\n"

static size_t convert_data_to_frame(char *buf, size_t buf_size, void *data,
                                    size_t size) {
  if (buf == NULL || data == NULL || size != sizeof(BoardData_t))
    return 0;
  ...
  int n = snprintf(
      ...
      "%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\n",
      bd->timestamp/1000, bd->temperature[0], bd->temperature[1], bd->temperature[2],
      bd->status_temp, bd->humidity, bd->pressure[0], bd->pressure[1],
      bd->pressure[2], bd->pressure[3], bd->pressure[4], bd->pressure[5],
      bd->pressure[6], bd->pressure[7], bd->voltage[0], bd->voltage[1],
      bd->voltage[2], bd->voltage[3], bd->voltage[4], bd->voltage[5],
      bd->voltage[6], bd->voltage[7]);
