...
static struct {
  ...
  SemaphoreHandle_t
      data_write_mutex; // prevent race condition during path changing
...
static void prepare_data_file_and_save(void) {
  ...
  while (received_data_counter < SD_MAX_DATA_RECEIVE) {
    ...
    size_t frame_size = mem.create_sd_frame_fnc(
        mem.data_buffer, sizeof(mem.data_buffer), item, item_size);
...
static bool check_sd_status(void) {
  ...
  void *item;
  size_t item_size;
  while ((item = xRingbufferReceive(mem.data_ringbuffer, &item_size, 0)) !=
         NULL) {
    vRingbufferReturnItem(mem.data_ringbuffer, item);
  }
  xSemaphoreTake(mem.ringbuffer_mutex, portMAX_DELAY);
...
static void log_check_and_save(void) {
  ...
  while (received_data_counter < SD_MAX_DATA_RECEIVE) {
    size_t item_size;
    const char *item =
        (const char *)xRingbufferReceive(mem.log_ringbuffer, &item_size, 0);
    if (item == NULL)
      break;
...
static bool create_unique_path(char *path, size_t size) {
  ...
  for (int i = 0; i < 1000; ++i) {
    ret = snprintf(temp_path, sizeof(temp_path), SD_MOUNT_POINT "/%s%d.txt",
                   path, i);
    if (ret == SD_PATH_SIZE) {
      return false;
    }
...
static bool initialize_task(sd_task_cfg_t *task_cfg) {
  ...
  // nie ma gwarancji, że w buforze będzie dokładnie
  // CONFIG_SD_DATA_RINGBUF_CAPACITY itemów, bo RINGBUF_TYPE_NOSPLIT dokłada do
  // każdego itemu jeszcze header (8 bajtów) oraz każdy item jest trzymany w
  // blokach po 32 bity (niezaleznie od rzeczywistego rozmiaru)
  mem.data_ringbuffer = xRingbufferCreate(
      (CONFIG_SD_DATA_RINGBUF_CAPACITY * (task_cfg->data_size)),
  ...
  mem.log_ringbuffer =
      xRingbufferCreate(SD_LOG_RINGBUF_SIZE, RINGBUF_TYPE_NOSPLIT);
  ...
  xTaskCreatePinnedToCore(sdTask, "sd task", task_cfg->stack_depth, NULL,
                          task_cfg->priority, &mem.sd_task, task_cfg->core_id);
