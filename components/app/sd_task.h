...
// RINGBUF_TYPE_NOSPLIT dokłada do każdego itemu header (8 bajtów), więc
// rzeczywista liczba itemów trzymanych w buforze jest mniejsza od
// CONFIG_SD_LOG_RINGBUF_CAPACITY
#define SD_LOG_RINGBUF_SIZE                                                    \
  (CONFIG_SD_LOG_RINGBUF_CAPACITY * (sizeof(char[SD_LOG_BUFFER_MAX_SIZE])))
...
typedef size_t (*create_sd_frame)(char *buffer, size_t buffer_size, void *data,
                                  size_t size);
