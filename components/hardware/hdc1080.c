...
static esp_err_t read_reg_8b(HDC1080_dev *hd1080, const uint8_t reg,
                             uint8_t *val) {
  bool ret = true;
  ret = hd1080->_i2c_write(hd1080->i2c_address, reg, 0, 0);
  uint8_t data = 0;
...
esp_err_t write_config_bits(HDC1080_dev *hd1080, const uint8_t mask,
  ...
  ret = write_reg_16b(hd1080, HDC1080_CONFIG_REG,
                      (config_old & ~(mask << offset)) | (val << offset));
  if (ret != ESP_OK) {
...
void hd1080_init(HDC1080_dev *dev) {
  uint16_t val;
  uint8_t v = 0;
  write_config_bits(dev, 1, MODE_OFFSET,
                    HDC1080_ACQUISITION_HUMIDITY_AND_TEMPERATURE);
  write_config_bits(dev, 1, TEMP_RES_OFFSET,
                    HDC1080_TEMPERATURE_RESOLUTION_14BIT);
