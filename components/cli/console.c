...
static void get_completions(const char *buf, linenoiseCompletions *lc) {
  ...
  for (size_t i = 0; i < command_count; i++) {
    ...
    if (!c->arg_completion || !c->cmd.argtable)
      continue;

    if (strlen(c->cmd.command) == cmd_len &&
        strncmp(cmd, c->cmd.command, cmd_len) == 0) {
      ...
      for (size_t j = 1; j < lc->len; j++) {
        ...
        while (k < prefix_len && cur[k] == first[k])
          k++;
        prefix_len = k;
        if (prefix_len == 0) {
  ...
  *bold = 0;   // normal (ANSI)
  ...
  if (lc.len == 1) {
    ...
    if (completion_split.token_len > line_split.token_len) {
      strncpy(hint_buf, completion_split.token + line_split.token_len,
              sizeof(hint_buf) - 1);
...
static void print_welcome_message() {
  if (linenoiseIsDumbMode()) {
    printf("\n============================================\n"
           ...
           "   Your terminal does NOT support ANSI escape sequences, so "
...
esp_err_t console_init(void) {
  ...
  BaseType_t task_ret = xTaskCreate(console_task, "console_task", 4096, NULL, 5,
                                    &console_task_handle);
...
esp_console_cmd_t *find_cmd_by_name(const char *name) {
  for (int i = 0; i < command_count; i++) {
    if (strcmp(command_list[i].cmd.command, name) == 0)
      return &command_list[i].cmd;
  }
...
esp_err_t print_cmd_usage(const char *name) {
  ...
  if (cmd) {
    ...
    if (cmd->argtable != NULL)
      arg_print_syntax(stdout, (void **)cmd->argtable, "\n");
